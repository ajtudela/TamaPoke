...
void onTap(int16_t x, int16_t y) {
  ...
  if (confirmUntil) {        // dialogo "soltar?": SI / NO
    if (timeLeft(confirmUntil) > 0 && x >= 118 && x <= 218 && y >= 252 && y <= 304) {
      pet.release();
    }
    confirmUntil = 0;
    return;
  ...
  if (feedMenuUntil) {       // selector de comida
    if (timeLeft(feedMenuUntil) > 0 && y >= 288 && y <= 352 && x >= 101 && x <= 365) {
      int item = (x - 101) / 66;
...
void render() {
  ...
  if (choiceKind) {
    if (timeLeft(choiceUntil) == 0) choiceKind = 0;
...
void stepGame() {
  uint32_t now = millis();
  // paso relativo a 85ms, el periodo de frame con el que se afinaron estas
  ...
  // cuanto tardara en dibujarse el frame anterior, que depende del tamano del
...
void sackTap() {
  if (timeLeft(sackUntil) == 0) return;  // ya termino el tiempo
  sackHits++;
...
void renderSack() {
  ...
  if (sackOverUntil) {
    if (timeLeft(sackOverUntil) == 0) { sackOpen = false; return; }
    char b[20];
  ...
  // barra de tiempo
  uint32_t left = timeLeft(sackUntil);
...
void drawBath() {
  ...
  uint32_t left = timeLeft(bathUntil);
  if (left > 800) {
